...
//    ② ⭐ 2026-08-19 에 자동 폴링을 없앴다가 2026-08-25 양호창님 지시로 «되살렸다».
//       지금은 10초 주기로 돈다(msTick() · MS_POLL_MS). 화면에 들어올 때와 앱으로
//       돌아올 때(visibilitychange) 각각 1회 조회하는 것도 그대로 있다.
//    ③ ⭐ ⏸ 정지 단추는 2026-08-25 양호창님 지시로 «없앴다». 폴링은 돈다.
//       ⛔ 옛 주석에 있던 「폴링을 되살리려면 정지 버튼도 함께 되살려야 한다. 둘은
//          한 쌍이다」는 «이제 틀린 규칙»이다. 지금은 폴링만 살아 있고 정지 단추는 없다.
//       ⭐ 그 규칙이 풀린 까닭 — 「자동 새로고침 정지 기능」 제거는 웹품질 검토에서
//          «승인»된 결정이다(2026-08-25). 근거 세 가지:
//            · KWCAG 2.2 «정지 기능 제공»의 «필수 예외»에 해당한다
//            · 10초마다 바뀌는 것은 «자료»일 뿐, 움직이는(모션) 콘텐츠가 아니다
//            · «저절로 멈추는» 3중 구조가 이미 있다 — 조회 중이면 겹쳐 쏘지 않고(msBusy),
//              화면이 가려지면 멈추고(document.hidden), 다른 화면에 있으면 멈춘다
//       ⛔ 그러므로 ⏸ 단추를 되살리지 말 것. 정본은 «디자인_규격서.md 19-8절»(세 앱에서
//          없앤 기능)과 «수정요구사항_2608/02_다음세션_인수인계_260825.md 4장»(웹품질
//          판정)이다. 여기에 옮겨 적지 말고 그쪽을 읽을 것.
//       (시민에게는 「지금 새로고침」도 필요 없다 — 10초마다 저절로 갱신된다)
...
//       ⚠ ⏸ 정지 단추는 2026-08-25 양호창님 지시로 «없앴다». 근거는 위 ③ 과
//         msPaintAutoBtn 주석에 있다(폴링은 그대로 돈다).
//
...
/* 🔄 자동 새로고침 주기 — ⭐ 2026-08-25 양호창님 지시로 «되살렸다».
   ...
   🔒 2026-08-19 에 없앤 «진짜» 이유는 트래픽 모양이었다 — 코드마다 1건씩 8초 폴링이라
      ...
      ② 이 화면이 «보일 때만» 돌고 document.hidden 이면 건너뜀, ③ 화면을 벗어나거나
      가리면 «저절로» 멈춤(2026-08-25 부터 ⏸ 단추는 없다 — 위 ③ 참조).
      곧 10초 주기라도 왕복은 «분당 6회»이고, 가짜 코드를 던지지 않는다.
   ⚠⚠ 2026-08-25 양호창님 지시로 ⏸ 정지 단추를 없앴다 — 폴링은 그대로 돌아간다.
      지침(KWCAG 2.2 «정지 기능 제공») 적합 근거는 msPaintAutoBtn 주석에 적어 두었다. */
const MS_POLL_MS = 10000;
/* ⚠ msAuto 는 2026-08-25 부터 «항상 true» 입니다 — 정지 단추를 없앴기 때문입니다.
...
/* 🔄 10초 주기 — «이 화면이 보일 때만». 다른 화면·백그라운드에서는 아무 일도 하지 않는다.
   ⭐ 2026-08-25 복원(양호창님 지시). 아래 네 가지 «건너뛰기»가 이 폴링의 안전장치다:
     ① msAuto 가 false 면 아무 것도 하지 않는다 — 다만 지금은 «항상 true» 이고 끄는
        길이 없다(⏸ 단추를 없앴다). 변수와 이 검사는 «남겨 둔» 것이다
...
function bindEvents() {
  ...
  /* ⛔ 여기에 있던 「⏸ 자동 새로고침 멈추기/켜기」 단추의 이벤트 등록은 «없앴습니다».
        2026-08-25 양호창님 지시(「사용자가 뭔지도 모르고 혼란스럽기만 할 거야」)로
        단추 자체를 뺐기 때문에, 걸어 줄 대상이 없습니다. 이 자리에는 지금 «코드가 없습니다».
     ★ 없앤 것은 «단추»뿐이고 자동 새로고침 «기능»은 그대로 돕니다 — msAuto 는 항상 true,
        10초 주기는 msTick()/MS_POLL_MS 가 담당합니다(위 msPaintAutoBtn 주석 참조).
     ⛔ ⏸ 단추도 #msRefresh(지금 새로고침)도 되살리지 마세요 — 없앤 까닭은
        `디자인_규격서.md` 19-8절(세 앱에서 없앤 기능)에 적혀 있습니다.
        되살리면 규격서 §0 「한 화면 버튼 3개 상한」에도 걸립니다(이 기기에서 지우기 ·
        확인 번호로 찾기 · 신청 취소 로 이미 찹니다). */
  // 「신청한 사업 / 제안한 정책」 갈래

...
(function () {
  ...
  var FREQ = ["청년", "노인·어르신", "농림축수산업", "소상공인·기업", "임신·출산", "영유아·보육", "주거·부동산", "귀농·귀촌"];
  ...
  function upgradeChips() {
    ...
    Array.prototype.forEach.call(chips, function (el) {
      // 맨 앞의 «전체» 칩은 분야가 아니라 «전체 사업 보기» 길이다(app.js 가 넣는다).
      // 접기 대상에서 빼야 항상 보인다 — 홈의 큰 버튼을 이 칩으로 옮겼기 때문이다.
      if (el.classList.contains("chip-all")) return;
      var name = plain(el.getAttribute("data-cat"));
      ...
      //      화면에서만 바꾸면 앱마다 달라 보인다.
      //   → 여기서는 «접기/펴기»(자주 찾는 8개)만 맡는다. 아이콘으로 바꾸지 말 것.
      if (FREQ.indexOf(name) === -1) { el.classList.add("chip-more"); hidden++; }
    });
    // 「분야 전체 보기」 토글 — .chip 이 아니므로 app.js 의 칩 클릭 처리와 섞이지 않는다
    var t = box.parentNode.querySelector(".chip-toggle");
    if (!hidden) { if (t) { t.parentNode.removeChild(t); } return; }
    if (!t) {
      t = document.createElement("button");
      t.type = "button";
      t.className = "chip-toggle tap";
      t.setAttribute("aria-controls", "categoryChips");
      t.addEventListener("click", function () {
        var open = box.classList.toggle("chips-open");
        t.setAttribute("aria-expanded", open ? "true" : "false");
        paint(t, open, hidden);
      });
      box.parentNode.insertBefore(t, box.nextSibling);
    }
    var isOpen = box.classList.contains("chips-open");
    t.setAttribute("aria-expanded", isOpen ? "true" : "false");
    paint(t, isOpen, hidden);
  }

  function paint(btn, open, hidden) {
    btn.innerHTML = svg("chev") +
      "<span>" + (open ? "분야 접기" : "분야 전체 보기 (" + hidden + ")") + "</span>";
